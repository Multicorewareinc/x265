...
#define BOOL(param, cliopt) \
    ...
    if (p->csvfn)
        s += snprintf(s, sizeof(s), " csv csv-log-level=%d", p->csvLogLevel);
    ...
    s += snprintf(s, sizeof(s), " input-res=%dx%d", p->sourceWidth - padx, p->sourceHeight - pady);
    ...
    s += snprintf(s, sizeof(s), " temporal-layers=%d", p->bEnableTemporalSubLayers);
    ...
    s += snprintf(s, sizeof(s), " lookahead-slices=%d", p->lookaheadSlices);
    ...
    s += snprintf(s, sizeof(s), " tu-inter-depth=%d", p->tuQTMaxInterDepth);
    s += snprintf(s, sizeof(s), " tu-intra-depth=%d", p->tuQTMaxIntraDepth);
    ...
    if (p->bEnableHME)
    {
        s += snprintf(s, sizeof(s), " Level 0,1,2=%d,%d,%d", p->hmeSearchMethod[0], p->hmeSearchMethod[1], p->hmeSearchMethod[2]);
        s += snprintf(s, sizeof(s), " merange L0,L1,L2=%d,%d,%d", p->hmeRange[0], p->hmeRange[1], p->hmeRange[2]);
    }
    ...
    if (p->bEnableLoopFilter)
        s += snprintf(s, sizeof(s), "=%d:%d", p->deblockingFilterTCOffset, p->deblockingFilterBetaOffset);
    ...
    if (p->recursionSkipMode == EDGE_BASED_RSKIP)
        s += snprintf(s, sizeof(s), " rskip-edge-threshold=%f", p->edgeVarThreshold);
    ...
    s += snprintf(s, sizeof(s), " rc=%s", p->rc.rateControlMode == X265_RC_ABR ? (
    ...
    if (p->rc.rateControlMode == X265_RC_ABR || p->rc.rateControlMode == X265_RC_CRF)
    {
        ...
        s += snprintf(s, sizeof(s), " qcomp=%.2f qpstep=%d", p->rc.qCompress, p->rc.qpStep);
        ...
        {
            s += snprintf(s, sizeof(s), " vbv-maxrate=%d vbv-bufsize=%d vbv-init=%.1f min-vbv-fullness=%.1f max-vbv-fullness=%.1f",
                p->rc.vbvMaxBitrate, p->rc.vbvBufferSize, p->rc.vbvBufferInit, p->minVbvFullness, p->maxVbvFullness);
            if (p->vbvBufferEnd)
                s += snprintf(s, sizeof(s), " vbv-end=%.1f vbv-end-fr-adj=%.1f", p->vbvBufferEnd, p->vbvEndFrameAdjust);
            if (p->rc.rateControlMode == X265_RC_CRF)
                s += snprintf(s, sizeof(s), " crf-max=%.1f crf-min=%.1f", p->rc.rfConstantMax, p->rc.rfConstantMin);   
        }
    }
    ...
    s += snprintf(s, sizeof(s), " aq-strength=%.2f", p->rc.aqStrength);
    ...
    if (p->vui.aspectRatioIdc == X265_EXTENDED_SAR)
        s += snprintf(s, sizeof(s), " sar-width : sar-height=%d:%d", p->vui.sarWidth, p->vui.sarHeight);
    s += snprintf(s, sizeof(s), " overscan=%d", p->vui.bEnableOverscanInfoPresentFlag);
    if (p->vui.bEnableOverscanInfoPresentFlag)
        s += snprintf(s, sizeof(s), " overscan-crop=%d", p->vui.bEnableOverscanAppropriateFlag);
    s += snprintf(s, sizeof(s), " videoformat=%d", p->vui.videoFormat);
    s += snprintf(s, sizeof(s), " range=%d", p->vui.bEnableVideoFullRangeFlag);
    s += snprintf(s, sizeof(s), " colorprim=%d", p->vui.colorPrimaries);
    s += snprintf(s, sizeof(s), " transfer=%d", p->vui.transferCharacteristics);
    s += snprintf(s, sizeof(s), " colormatrix=%d", p->vui.matrixCoeffs);
    s += snprintf(s, sizeof(s), " chromaloc=%d", p->vui.bEnableChromaLocInfoPresentFlag);
    if (p->vui.bEnableChromaLocInfoPresentFlag)
        s += snprintf(s, sizeof(s), " chromaloc-top=%d chromaloc-bottom=%d",
        p->vui.chromaSampleLocTypeTopField, p->vui.chromaSampleLocTypeBottomField);
    s += snprintf(s, sizeof(s), " display-window=%d", p->vui.bEnableDefaultDisplayWindowFlag);
    ...
    if (p->masteringDisplayColorVolume)
        s += snprintf(s, sizeof(s), " master-display=%s", p->masteringDisplayColorVolume);
    if (p->bEmitCLL)
    ...
    s += snprintf(s, sizeof(s), " analysis-reuse-level=%d", p->analysisReuseLevel);
    s += snprintf(s, sizeof(s), " analysis-save-reuse-level=%d", p->analysisSaveReuseLevel);
    s += snprintf(s, sizeof(s), " analysis-load-reuse-level=%d", p->analysisLoadReuseLevel);
    ...
    s += snprintf(s, sizeof(s), " refine-ctu-distortion=%d", p->ctuDistortionRefine);
    BOOL(p->bLimitSAO, "limit-sao");
    s += snprintf(s, sizeof(s), " ctu-info=%d", p->bCTUInfo);
    BOOL(p->bLowPassDct, "lowpass-dct");
    s += snprintf(s, sizeof(s), " refine-analysis-type=%d", p->bAnalysisType);
    s += snprintf(s, sizeof(s), " copy-pic=%d", p->bCopyPicToFrame);
    s += snprintf(s, sizeof(s), " max-ausize-factor=%.1f", p->maxAUSizeFactor);
    BOOL(p->bDynamicRefine, "dynamic-refine");
    BOOL(p->bSingleSeiNal, "single-sei");
    BOOL(p->rc.hevcAq, "hevc-aq");
    BOOL(p->bEnableSvtHevc, "svt");
    BOOL(p->bField, "field");
    s += snprintf(s, sizeof(s), " qp-adaptation-range=%.2f", p->rc.qpAdaptationRange);
    s += snprintf(s, sizeof(s), " scenecut-aware-qp=%d", p->bEnableSceneCutAwareQp);
    if (p->bEnableSceneCutAwareQp)
        s += snprintf(s, sizeof(s), " fwd-scenecut-window=%d fwd-ref-qp-delta=%f fwd-nonref-qp-delta=%f bwd-scenecut-window=%d bwd-ref-qp-delta=%f bwd-nonref-qp-delta=%f", p->fwdMaxScenecutWindow, p->fwdRefQpDelta[0], p->fwdNonRefQpDelta[0], p->bwdMaxScenecutWindow, p->bwdRefQpDelta[0], p->bwdNonRefQpDelta[0]);
    s += snprintf(s, sizeof(s), " conformance-window-offsets right=%d bottom=%d", p->confWinRightOffset, p->confWinBottomOffset);
    s += snprintf(s, sizeof(s), " decoder-max-rate=%d", p->decoderVbvMaxRate);
    BOOL(p->bliveVBV2pass, "vbv-live-multi-pass");
    if (p->filmGrain)
        s += snprintf(s, sizeof(s), " film-grain=%s", p->filmGrain); // Film grain characteristics model filename
    if (p->aomFilmGrain)
        s += snprintf(s, sizeof(s), " aom-film-grain=%s", p->aomFilmGrain);
